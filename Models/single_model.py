...
class JCS(nn.Module):
    def __init__(self, pretrained=None, use_carafe=True,
        ...
        self.cls1 = nn.Conv2d(dec_channels[0], 4, 1, stride=1, padding=0)
        self.cls2 = nn.Conv2d(dec_channels[1], 4, 1, stride=1, padding=0)
        self.cls3 = nn.Conv2d(dec_channels[2], 4, 1, stride=1, padding=0)
        self.cls4 = nn.Conv2d(dec_channels[3], 4, 1, stride=1, padding=0)
        self.cls5 = nn.Conv2d(dec_channels[4], 4, 1, stride=1, padding=0)
    ...
    def forward(self, input):
        ...
        for idx, feature in enumerate(features[:-1]):
            saliency_maps.append(torch.unsqueeze(F.interpolate(
        ...
        print("Saliency shape: ", saliency_maps[0].shape, len(saliency_maps)) 
