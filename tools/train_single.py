...
#         myTransforms.RandomFlip(),
    ...
    valLoader = torch.utils.data.DataLoader(
        Dataset(args.data_dir, 'test', transform=b_transform),
    ...
    if os.path.isfile(log_file):
        ...
        logger.write("\n%s\t\t%s\t%s\t%s\t%s\t%s\t\t%s\t\t%s\t\t%s\t\t%s\t\t%s" % ('Epoch', \
                'Loss(Tr)', 'IoU (tr)', 'MAE (tr)', 'IoU (val)', 'MAE (val)', 'lr', 'AUC_ROC (tr)', 'AUC_ROC (val)', 'AUC_PR (tr)', 'AUC_PR (val)'))
    logger.flush()

    optimizer = torch.optim.Adam(model.parameters(), args.lr, (0.9, 0.999), eps=1e-08, weight_decay=1e-4) #changed 1e-4
...
#             torch.save(model.state_dict(), model_file_name)
        ...
        if auc_roc_val > best_auc_roc_val:
            print("found a good model > 0.7, start to save that!")
            torch.save(model.state_dict(), model_file_name)
            best_auc_roc_val = auc_roc_val

        logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.7f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f" % (epoch, loss_tr, IoU_tr, MAE_tr, IoU_val, MAE_val, lr, auc_roc_tr, auc_roc_val, auc_pr_tr, auc_pr_val))
        logger.flush()
        print("Epoch " + str(epoch) + ': Details')
        print("\nEpoch No. %d:\tTrain Loss = %.4f\tVal Loss = %.4f\t IoU(tr) = %.4f\t IoU(val) = %.4f\t AUC_R(tr) = %.4f\t AUC_R (val) = %.4f\t AUC_P (val) = %.4f" \
                % (epoch, loss_tr, loss_val, IoU_tr, IoU_val, auc_roc_tr, auc_roc_val, auc_pr_val))
        torch.cuda.empty_cache()
    logger.close()
